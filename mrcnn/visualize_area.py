...
def display_instances_10fps(image, boxes, masks, class_ids, class_names, 
    scores, colors, real_time=True, area=0):
    N = boxes.shape[0]
    #if not N:
    #    print('\n*** No instances to display *** \n')
    #else:
    ...
    for i in range(N):
        ...
        score = scores[i]
        label = class_names[class_id]
        caption = '{} {:.2f}|{} cm^2'.format(label, score, area) if score else label
        ...
        cv2.putText(masked_image, caption, (x1, y1 + ret[1]),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, gentle_grey, 1, lineType=cv2.LINE_AA)
